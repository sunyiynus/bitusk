...
class MsgTyper {
 public:
  ...
  static ustring port;
  static std::map<int, boost::function<bool(Peer&, Peer&, const ustring&)>>
      processors;
  ...
  static const std::string ConvertUstringToString(
  ...
  static bool ProcessInterestedMsg(Peer& myself, Peer& peer,
                                   const ustring& str);
  static bool ProcessUninterestedMsg(Peer& myself, Peer& peer,
                                     const ustring& str);
...
inline long long MsgTyper::CharToInt(const ustring& str) {
  if (str.size() < 4)
    return 0;
  long long i = 0;
  i = str[0] * 256 * 256 * 256 + str[1] * 256 * 256 + str[2] * 256 + str[3];
