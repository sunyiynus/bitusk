...
std::map<int, boost::function<bool(Peer&, Peer&, const MsgTyper::ustring&)>>
    MsgTyper::processors;
...
const std::string BasicCreateMsg(
    Peer& myself, Peer& peer,
    boost::function<void(Peer&, Peer&,
...
const std::string MsgTyper::CreateHaveMsg(Peer& myself, Peer& peer,
                                          size_t index) {
  uostringstream msg;
  IntToChar(5, msg);
  msg << UCHAR(4);
...
const std::string MsgTyper::CreateBitfieldMsg(Peer& myself, Peer& peer) {
  uostringstream msg;
  IntToChar(
      1 + ((myself.bitmap.Size() % 8 == 0) ? (myself.bitmap.Size() / 8)
                                           : (myself.bitmap.Size() / 8 + 1)),
      msg);
  msg << UCHAR(5);
  BitMap& bitmap = myself.bitmap;
  int charlen =
      (bitmap.Size() % 8 == 0) ? (bitmap.Size() / 8) : (bitmap.Size() / 8 + 1);
...
bool MsgTyper::ProcessHandShakeMsg(Peer& myself, Peer& peer,
                                   const ustring& str) {

  return true;
}

bool MsgTyper::ProcessKeepAliveMsg(Peer& myself, Peer& peer,
                                   const ustring& str) {
...
bool MsgTyper::ProcessInterestedMsg(Peer& myself, Peer& peer,
                                    const ustring& str) {

  return true;
}

bool MsgTyper::ProcessUninterestedMsg(Peer& myself, Peer& peer,
                                      const ustring& str) {
...
bool MsgTyper::ProcessBitfieldMsg(Peer& myself, Peer& peer,
                                  const ustring& str) {
