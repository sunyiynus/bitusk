...
FilesManagerSV* FilesManagerSV::GetInstance(const std::string& dd,
...
FilesManagerSV::FilesManagerSV(const std::string& dd,
...
bool FilesManagerSV::LoadingFiles() {
  ...
  fs_ = std::make_shared<std::fstream>(
      download_dir_ + files_info_[0].path + ".tmp",
...
void FilesManagerSV::ReadSliceNodeFromDisk(Node& node) {
  ...
  if (bytes > 0) {
    node.buffer = ustring(reinterpret_cast<unsigned char*>(buffer.get()),
                          node.buffer.size());
  } else {
    node.buffer.clear();
  }
...
const Node FilesManagerSV::ReadSliceNodeFromDisk(size_t index, size_t offset,
                                                 size_t length) {
...
bool FilesManagerSV::WriteSliceNodeToDisk(const Node& node) {
  ...
  fs_->write(reinterpret_cast<const char*>(node.buffer.c_str()),
             node.buffer.size());
  fs_->flush();
  return true;
}
...
const Node FilesManagerDecorator::ReadSliceNodeFromDisk(const size_t index,
                                                        const size_t offset,
                                                        const size_t length) {
  ASSERT_NOT_NULPTR(fmanager_);
...
const PieceNode FilesManagerDecorator::ReadPieceFromDisk(const size_t index,
                                                         const size_t piece_n) {
  ASSERT_NOT_NULPTR(fmanager_);
  PieceNode pnode;
